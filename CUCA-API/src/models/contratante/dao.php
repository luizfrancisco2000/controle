...
{
    $str = $db->prepare('SELECT * FROM contratante a WHERE a.id=id');
    $str->bindParam("id", $id);
    $str->execute();
    $contratantes = $str->fetchAll();
    if (count($contratantes) > 0) {
...
{
    $str = $db->prepare('SELECT * FROM contratante a WHERE a.nome=:nome');
    $str->bindParam("id", $nome);
    $str->execute();
    $contratantes = $str->fetchAll();
    if (count($contratantes) > 0) {
        return $contratantes[0];
...
{
    $str = $db->prepare('select e.nome, c.turma, c.horario, e.ativo, m.pago from contrato c, contratante e, mensalidade m WHERE e.FKidContrato=c.id and m.FKidContrato=c.id and m.mes=date_format(NOW(),"%m")');
    $str->execute();
    $contratantes = $str->fetchAll();
    if (count($contratantes) > 0) {
        return $contratantes;
...
function procurar_todos_contratante($db)
{
    $str = $db->prepare('SELECT * FROM contratante c');
    $str->execute();
    $contratantes = $str->fetchAll();
    if (count($contratantes) > 0) {
        return $contratantes;
